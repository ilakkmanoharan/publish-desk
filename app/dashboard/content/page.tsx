...
export default function ContentLibraryPage() {
  ...
  const contentWithMeta = (
    ...
    tags: (c.tagIds || [])
      .map((tid) => (allTags as { id: string; name: string }[]).find((t) => t.id === tid))
      .filter(Boolean)
    ...
    <div className="mx-auto w-full max-w-6xl">
      <div className="rounded-2xl border border-border bg-card p-6 shadow-md md:p-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <h1 className="font-display text-2xl font-bold tracking-tight text-foreground">Content library</h1>
          <Link
            href="/dashboard/content/new"
            className="rounded-xl bg-accent px-4 py-2.5 text-sm font-medium text-white no-underline transition-opacity hover:opacity-90"
          >
            New content
          </Link>
        </div>
